...
- Atlas image: `assets/atlas.png`. It's produced by `scripts/buildAtlas.ts` from the files in `assets/textures/`.
- `src/App.ts` currently hardcodes `AtlasRows` / `AtlasCols` (default 3x3). Ensure those match the atlas grid produced by `buildAtlas`.
- The `build:atlas` npm script runs `tsx scripts/buildAtlas.ts`. If `tsx` is not available, install it as a dev dependency (`npm i -D tsx`) or compile the script with `tsc` and run with `node`.
